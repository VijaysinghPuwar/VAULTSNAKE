...
export default function AdminLogsPage() {
  ...
  return (
    <div className="p-6 animate-fade-in">
      <div className="mb-6">
        <h1 className="text-xl font-bold text-cyber-text">Audit Logs & Alerts</h1>
        <p className="text-xs text-cyber-muted mt-1">Platform-wide security event history</p>
      </div>
      ...
      <div className="flex gap-2 mb-6">
        {(["logs", "alerts"] as Tab[]).map((t) => (
          <button
            ...
            onClick={() => setTab(t)}
            className={clsx(
              "px-4 py-2 rounded text-sm transition-colors capitalize",
                ...
                <tbody>
                  {logs.map((log) => (
                    <tr key={log.id}>
                      ...
                      <td className="text-cyber-muted text-xs">{log.user_email}</td>
                ...
                <tbody>
                  {alerts.map((alert) => (
                    <tr key={alert.id}>
                      ...
                      <td className="text-cyber-muted text-xs">{(alert as import("@/types").Alert & { user_email?: string }).user_email ?? "—"}</td>
                      <td className="text-xs text-cyber-text">{alert.alert_type.replace(/_/g, " ")}</td>
                      <td className="text-cyber-text max-w-xs truncate text-xs">{alert.description}</td>
